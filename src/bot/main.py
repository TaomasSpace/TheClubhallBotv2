...
@bot.tree.command(name="fakeban", description="Create a fake ban message to confuse members")
async def fakeBanCommand(interaction: discord.Interaction, user: discord.Member, reason: str):
    await fakeBan.fakeBan(interaction=interaction, user=user, reason=reason)

# ───── Coin Management (Admin) ─────
@bot.tree.command(name="addmoney", description="Give coins to a user (Admin/Owner only)")
async def addMoneyCommand(interaction: discord.Interaction, user: discord.Member, amount: int):
    await addMoney.addMoney(interaction=interaction, user=user, amount=amount)

@bot.tree.command(name="remove", description="Remove coins from a user (Admin/Owner only)")
async def removeCommand(interaction: discord.Interaction, user: discord.Member, amount: int):
    await removeMoney.remove(interaction=interaction, user=user, amount=amount)

@bot.tree.command(name="addservermoney", description="Add coins to the server/bank balance")
...
async def requestCommand(interaction: discord.Interaction, user: discord.Member, amount: int, reason: str):
    await request.request(interaction=interaction, user=user, amount=amount, reason=reason)

@bot.tree.command(name="donate", description="Send coins to another user")
async def donateCommand(interaction: discord.Interaction, user: discord.Member, amount: int):
